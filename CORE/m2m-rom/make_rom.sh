...
##############################################################################
# gbc4mega65 specific: Generate osm_const.asm
#
# The firmware never hardcodes menu line numbers or menu group ids: they are
# scraped from the C_MENU_* constants in ../vhdl/mega65.vhd and from the
# OPTM_G_* constants in ../vhdl/config.vhd (single source of truth is the
# VHDL). Keep those constants single-line for the awk scrapers below.
##############################################################################
...

{
    ...
    echo ""
    echo "; -----------------------------------------------------------------"
    echo "; gbc4mega65 specific OSM item constants"
    echo "; -----------------------------------------------------------------"
    echo ""
} > osm_const.asm

awk '/constant C_MENU_/ {gsub("C_MENU_", "GBC_OSM_"); gsub(";", ""); printf("%-31s .EQU %s\n", $2, $6)}' ../vhdl/mega65.vhd >> osm_const.asm

{
    echo ""
    echo "; -----------------------------------------------------------------"
    echo "; gbc4mega65 specific menu group items"
    echo "; -----------------------------------------------------------------"
    echo ""
} >> osm_const.asm

awk '/constant OPTM_G_/ && !/16#/ {gsub("OPTM_G_", "GBC_OPTM_G_"); gsub(";", ""); printf("%-31s .EQU %s\n", $2, $6)}' ../vhdl/config.vhd >> osm_const.asm

{
    echo ""
    echo "; -----------------------------------------------------------------"
    echo "; gbc4mega65 specific QNICE device ids"
    echo "; -----------------------------------------------------------------"
    echo ""
} >> osm_const.asm

awk '/constant C_DEV_GB_CART/ {gsub(/.*x"/, ""); gsub(/".*/, ""); printf("%-31s .EQU 0x%s\n", "GBC_DEV_CART", $0)}' ../vhdl/globals.vhd >> osm_const.asm
awk '/constant C_DEV_GB_BIOS/ {gsub(/.*x"/, ""); gsub(/".*/, ""); printf("%-31s .EQU 0x%s\n", "GBC_DEV_BIOS", $0)}' ../vhdl/globals.vhd >> osm_const.asm
...
##############################################################################
# Robustness guards: fail loudly if the assembly failed or if the ROM
# exceeds the usable QNICE ROM area (0x0000 - 0x6FFF = 28672 words;
# 0x7000 and above is QNICE MMIO space)
##############################################################################

if [ ! -f m2m-rom.rom ]; then
    echo ""
    echo "ERROR: m2m-rom.rom was not generated - the assembly failed."
    echo ""
    exit 1
fi

rom_words=$(wc -l < m2m-rom.rom | tr -d ' ')
if [ "$rom_words" -gt 28672 ]; then
    echo ""
    echo "ERROR: m2m-rom.rom is $rom_words words large, but the usable"
    echo "QNICE ROM area is only 28672 words (0x0000 - 0x6FFF)."
    echo ""
    exit 1
fi

echo "make_rom.sh: OK, m2m-rom.rom has $rom_words words."
